...
void render_array(const KeyVec& v, size_t max_display,
    ...
    for (size_t i = 0; i < v.size(); i += step) {
        ...
        std::string bar(static_cast<size_t>(std::max(0, height)), hi ? '|' : ':');
...
void simulate_radix(const KeyVec& input, bool slow) {
    ...
    for (int pass = 0; pass < 4; ++pass) {
        ...
        for (size_t b = 0; b < 16; ++b) std::cout << "[" << b << "]=" << cnt[b] << " ";
        std::cout << "…\n";

        std::vector<int> pre(256, 0);
        for (size_t b = 1; b < 256; ++b) pre[b] = pre[b-1] + cnt[b-1];
        std::cout << "    Prefix sum computed → scattering …\n";
        pace(slow);

        for (size_t i = 0; i < n; ++i) {
            const size_t b = static_cast<size_t>((src[i] >> sh) & 0xFF);
            dst[static_cast<size_t>(pre[b]++)] = src[i];
        }
        std::swap(src, dst);
        pace(slow);
    }
...
void simulate_sample(const KeyVec& input, bool slow) {
    ...
    std::vector<int> splitters(static_cast<size_t>(p - 1));
    for (int i = 0; i < p - 1; ++i)
        splitters[static_cast<size_t>(i)] = sample[std::min(sample.size() - 1, static_cast<size_t>(i + 1) * 10u)];
    ...
    for (int k : v) {
        const int b = static_cast<int>(std::lower_bound(splitters.begin(),
                                             splitters.end(), k)
                            - splitters.begin());
        buckets[static_cast<size_t>(b)].push_back(k);
    }
    for (size_t b = 0; b < static_cast<size_t>(p); ++b)
