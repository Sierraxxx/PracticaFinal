...
static Extent compute_extent(const KeyVec& v) noexcept {
    ...
    // Parallel min/max reduction — each thread computes local extent,
    // then we reduce across threads. Avoids the O(n) sequential bottleneck.
    std::vector<int> local_mn(p, v.front());
    std::vector<int> local_mx(p, v.front());
    ...
    e.mn = *std::min_element(local_mn.begin(), local_mn.end());
    e.mx = *std::max_element(local_mx.begin(), local_mx.end());
...
static bool run_crn(KeyVec& v, const Extent& e,
    ...
    // Parallelized over the U dimension: each thread owns a slice [lo, hi)
    // of the histogram and sums across all p local histograms for that slice.
    // For large U this converts p×U sequential ops into p×(U/p) parallel ops.
    std::vector<int> H(U, 0);
    pu::parallel_chunks(U, p, [&](size_t lo, size_t hi, unsigned /*tid*/) noexcept {
